...
def _ssim_s(img1, img2, window, window_size, channel, size_average = True):
    mu1 = F.conv2d(img1, window, padding = window_size//2, groups = channel)
    mu2 = F.conv2d(img2, window, padding = window_size//2, groups = channel)
...
class DSSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if channel == self.channel and self.window.data.type() == img1.data.type():
            ...
            self.channel = channel
        if self.mode == 'all':
            ssim_v = _ssim(img1, img2, window, self.window_size, channel, self.size_average)
        elif self.mode == 'c_s':
            ssim_v = _ssim_c_s(img1, img2, window, self.window_size, channel, self.size_average)
        elif self.mode == 's':
            ssim_v = _ssim_s(img1, img2, window, self.window_size, channel, self.size_average)
        else:
            ssim_v = _ssim_l(img1, img2, window, self.window_size, channel, self.size_average)
        return (1-ssim_v)/2