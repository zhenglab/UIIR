...
class EUVPDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain==True:
            ...
            self.trainfile = 'datasets/euvp_train.txt'
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root,'inputs',line.rstrip()))
        elif opt.isTrain==False:
            #self.real_ext='.jpg'
            print('loading test file')
            self.trainfile = 'datasets/euvp_test.txt'
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root,'inputs',line.rstrip()))
    ...
    def __getitem__(self, index):
        ...
        target_path = self.image_paths[index].replace('inputs','real')
        ...
        # if self.isTrain:
        #     path = path.replace(name_parts[-1],name_parts[-2]+"_"+name_parts[-1])
