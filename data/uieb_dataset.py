...
class UIEBDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain==True:
            ...
            self.trainfile = 'datasets/uieb_train.txt'
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root, 'uieb_train',line.rstrip()))
        elif opt.isTrain==False:
            #self.real_ext='.jpg'
            print('loading test file')
            self.trainfile = 'datasets/uieb_test.txt'
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root, 'uieb_test',line.rstrip()))
    ...
    def __getitem__(self, index):
        ...
        # target_path = self.image_paths[index].replace(name_parts[-2],'testB').replace(name_parts[-1], name_parts[-1].lstrip('0'))
        if self.isTrain:
            target_path = self.image_paths[index].replace('uieb_train','gt_train')
        else:
            target_path = self.image_paths[index].replace('uieb_test','gt_test')
